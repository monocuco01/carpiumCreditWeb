...
// IMPORTANTE: Asegúrate de que el nombre del archivo coincida con tu asset (png, jpg, etc.)
...
const Hero = () => {
  return (
    <section className="relative pt-36 pb-20 px-6 overflow-hidden bg-[#001A33]">
      ...
      <div className="absolute bottom-10 right-1/4 w-64 h-64 bg-[#E5C100]/5 rounded-full blur-[100px] -z-10"></div>
      ...
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-left-10 duration-700">
          <div className="inline-flex items-center gap-2.5 bg-[#E5C100]/10 border border-[#E5C100]/20 px-4 py-2 rounded-full shadow-[0_0_15px_rgba(229,193,0,0.1)]">
            ...
            <span className="text-[#E5C100] text-xs font-bold uppercase tracking-widest">Personalized Elite Solutions</span>
          </div>
          ...
          <p className="text-gray-400 text-lg md:text-xl max-w-lg leading-relaxed italic">
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-2">
            <button className="bg-[#E5C100] text-[#001A33] px-10 py-4.5 rounded-xl font-bold text-sm uppercase tracking-wider hover:bg-yellow-500 transition-all shadow-[0_10px_30px_rgba(229,193,0,0.25)] flex items-center justify-center gap-2.5 group">
              Start Your Journey
              <ChevronRight size={18} className="group-hover:translate-x-1.5 transition-transform" />
            </button>
            <button className="border-2 border-gray-700 text-white px-10 py-4.5 rounded-xl font-bold text-sm uppercase tracking-wider hover:bg-white/5 hover:border-white transition-all">
              View Our Process
            </button>
          </div>

          <div className="flex flex-wrap items-center gap-x-8 gap-y-4 pt-6 border-t border-gray-800/60 max-w-lg">
        ...
        {/* --- PARTE VISUAL: FOTO DEL DUEÑO (NUEVO) --- */}
        <div className="relative animate-in fade-in zoom-in duration-1000 lg:pl-10">
          
          {/* Marco decorativo "Elite" detrás de la foto */}
          <div className="absolute -inset-4 border-2 border-[#E5C100]/20 rounded-3xl -z-10 scale-95 group-hover:scale-100 transition-transform duration-500"></div>
          
          <div className="relative bg-[#0a2540] p-2 rounded-3xl border border-white/10 shadow-2xl overflow-hidden group">
            
            {/* Imagen del dueño con zoom suave al hover */}
            <img 
              src={photoOwner} 
            ...
            {/* Efecto de luz dorada superior (Brillo de 'Elite') */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#E5C100]/15 rounded-full blur-3xl opacity-60 group-hover:opacity-100 transition-opacity"></div>
            
            {/* Overlay sutil inferior para legibilidad del nombre */}
            <div className="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-[#001A33] to-transparent rounded-b-[20px]"></div>

            {/* Etiqueta con Nombre y Cargo (Opcional, pero recomendado) */}
